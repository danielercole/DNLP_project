...
from cyclegan_tst.models.CycleGANModelDO import CycleGANModel
...
# MODIFICA: aggiorna max_sequence_length da 64 a 256 (o valore appropriato)
parser.add_argument('--max_sequence_length', type=int,  dest="max_sequence_length", default=256, help='Max sequence length')
...
parser.add_argument('--generator_model_tag', type=str, dest="generator_model_tag", help='The tag of the model for the generator (e.g., the GPT2 Instruct model).')
...
if args.nonparal_same_size:
    ...
    if mono_ds_a_len > mono_ds_b_len:
        mono_ds_a.reduce_data(mono_ds_b_len)
    else:
        mono_ds_b.reduce_data(mono_ds_a_len)

if args.n_references is not None:
    parallel_ds_evalAB = ParallelRefDataset(
        dataset_format='line_file',
        style_src=style_a,
        style_ref=style_b,
    ...
    parallel_ds_evalBA = ParallelRefDataset(
        dataset_format='line_file',
        style_src=style_b,
        style_ref=style_a,
...
for epoch in range(start_epoch, args.epochs):
    ...
    for unsupervised_a, unsupervised_b in zip(mono_dl_a, mono_dl_b):
        ...
        # Dummy classification metrics / BERTScore computation to check GPU memory usage
